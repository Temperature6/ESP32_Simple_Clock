...
const char *privateKey = "Sg68RU5VOEzrlcpsO";
const char *city = "Baoding";
...
long long GetTimeStamp()
{
    if (TEST_ENABLE)
    {
        return 1673490967; //2023-01-12 10:36:07
    }
    HTTPClient client;
...
void GetWeather(Weather_data* w_data, String city)
{
    if (TEST_ENABLE)
    {
        ...
        w_data->temp_high = 27;
        w_data->temp_low = 14;
        w_data->weat_day = "Weather Test";
        w_data->weat_night = "Weather Test";
        w_data->success = 1;
        return;
    }
    WiFiClient client;
    ...
    int temp_high = doc["results"][0]["daily"][0]["high"];
    int temp_low = doc["results"][0]["daily"][0]["low"];
    int code_day = doc["results"][0]["daily"][0]["code_day"];
    int code_night = doc["results"][0]["daily"][0]["code_night"];
    
    w_data->code_day = code_day;
    w_data->code_night = code_night;
    w_data->temp_high = temp_high;
    w_data->temp_low = temp_low;
